-- ============================================
-- URGENT: Fix Schema for GymVision AI
...
-- Step 2: Check if workouts table has date column, if not add it
DO $$ 
BEGIN
  ...
  -- Add exercises column if it doesn't exist (for backward compatibility)
  IF NOT EXISTS (
    SELECT 1 
    FROM information_schema.columns 
    WHERE table_schema = 'public' 
...
-- Step 5: Create policies for weights (drop only if they exist to avoid errors)
DO $$ 
BEGIN
...
-- Step 6: Create policies for workouts (drop only if they exist to avoid errors)
DO $$ 
BEGIN
...
-- Step 7: Create policies for workout_exercises (drop only if they exist to avoid errors)
DO $$ 
BEGIN
...
-- Step 9: Refresh schema cache (this is important!)
